 ...
 * (A004 만료 / A003 위조)를 그대로 내려준다. 셋을 모두 A001 로 뭉치면 클라이언트가 "만료면 조용히
 * 재발급, 그 밖에는 재로그인"을 구분할 수 없다.
 *
 * <p>토큰 자체가 없는 요청은 검증할 것이 없으므로 A001 이다.
 */
@Component
public class RestAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    /**
     * 필터가 남긴 검증 실패 사유를 쓰되, 401 이 아닌 코드가 실려 오면 무시한다 — 상태(401)와 코드의
     * 의미가 어긋난 응답이 나가지 않게 한다.
     */
