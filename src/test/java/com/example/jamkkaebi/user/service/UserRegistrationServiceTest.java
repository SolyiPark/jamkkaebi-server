...
class UserRegistrationServiceTest {
    ...
    void registersOnFirstLogin() {
        User user = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "g-1001", "잠깨비").user();
    ...
    void neverStoresEmail() {
        User user = userRegistrationService.findOrRegister(AuthProvider.KAKAO, "k-1002", "잠깨비").user();
    ...
    void reusesExistingUser() {
        User first = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "g-1003", "잠깨비").user();
        User second = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "g-1003", "바뀐이름").user();
    ...
    void separatesAccountsByProvider() {
        User google = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "same-id", "구글잠깨비").user();
        User kakao = userRegistrationService.findOrRegister(AuthProvider.KAKAO, "same-id", "카카오잠깨비").user();
    ...
    void survivesOverlongNickname() {
        User user = userRegistrationService.findOrRegister(
                AuthProvider.KAKAO, "k-1004", "열두글자가넘어가는아주긴닉네임").user();
    ...
    void survivesMissingNickname() {
        User user = userRegistrationService.findOrRegister(AuthProvider.KAKAO, "k-1005", null).user();
    ...
    void issuesDistinctFriendCodes() {
        User a = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "g-2001", "가").user();
        User b = userRegistrationService.findOrRegister(AuthProvider.GOOGLE, "g-2002", "나").user();
